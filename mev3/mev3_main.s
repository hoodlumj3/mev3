...
_main:
	...
	call	_Malloc_CHIP
	move.l	d0,_Project_Node

	move.w	#0,_Project_Set-PC(gl)
	move.w	#0,_Map_Set-PC(gl)
	move.w	#0,_Tile_Set-PC(gl)
	move.w	#0,_Palette_Set-PC(gl)
	move.w	#0,_Shape_Set-PC(gl)
;	move.w	#0,_Shape_Edit-PC(gl)

	call.l	_Add_Project_Node
	call.l	_Read_Project_Node

	move.w	#-1,Last_Prog_Section-PC(gl)
	move.w	#-1,Prev_Prog_Section-PC(gl)
	move.w	#SECTION_MAP,Run_Prog_Section-PC(gl)
;	move.w	#SECTION_TILE,Run_Prog_Section-PC(gl)

_main_running_loop:
	moveq.l	#0,d0
	move.w	Run_Prog_Section-PC(gl),d0	; running section
	move.w	d0,Last_Prog_Section-PC(gl)
	tst.w	d0
...
_main_setup:
	...
	lea	Prog_Section_Setup-PC(gl),a0
	call	Do_Designated_Execution
_main_run:
		;		  *************
		;		   run section
		;		  *************

	lea	Prog_Section_Run-PC(gl),a0
	call	Do_Designated_Execution
_main_shutown:
		;		******************
		;		 shutdown section
		;		******************
	lea	Prog_Section_Shutdown-PC(gl),a0
	call	Do_Designated_Execution
	tst.w	Last_Prog_Section-PC(gl)
	bmi.s	.dont_save_prev
	move.w	d0,Prev_Prog_Section-PC(gl)	
.dont_save_prev
	btst	#7,$BFE001
...
_main_shut_down:
;	jsr	_Remove_All_Shape_Nodes
;	jsr	_Remove_All_Palette_Nodes
;	jsr	_Remove_All_Tile_Nodes
;	jsr	_Remove_All_Map_Nodes

	call	_Remove_All_Project_Nodes

;	jsr	Dispose_All_Work_Images
;	jsr	Dispose_All_Work_Borders
;	jsr	Dispose_All_Work_Texts



	call	Close_Libraries	
...
Prog_Section_Setup:
			DC.L	Setup_None			; 0Project
			DC.L	Setup_Map_Ed			; 1Map
			DC.L	Setup_Tile_Ed			; 2Tile
			DC.L	Setup_None			; 3Palette
			DC.L	Setup_None			; 4Shape
			DC.L	Setup_None			; 5Anim
			DC.L	Setup_None			; 6Copper
			DC.L	_Setup_Preferences		; 7Prefs
			DC.L	_Setup_File			; 8File
Prog_Section_Run:
			DC.L	Handle_None_Messages		; 0Project
			DC.L	Handle_Map_Ed_Messages		; 1Map
			DC.L	Handle_Tile_Ed_Messages		; 2Image
			DC.L	Handle_None_Messages		; 3Palette
			DC.L	Handle_None_Messages		; 4Shape
			DC.L	Handle_None_Messages		; 5Anim
			DC.L	Handle_None_Messages		; 6Copper
			DC.L	_Handle_Preferences_Messages	; 7Prefs
			DC.L	_Handle_File_Messages		; 8File
Prog_Section_Shutdown:
			DC.L	Shutdown_None			; 0Project
			DC.L	Shutdown_Map_Ed			; 1Map
			DC.L	Shutdown_Tile_Ed		; 2Image
			DC.L	Shutdown_None			; 3Palette
			DC.L	Shutdown_None			; 4Shape
			DC.L	Shutdown_None			; 5Anim
			DC.L	Shutdown_None			; 6Copper
			DC.L	_Shutdown_Preferences		; 7Prefs
			DC.L	_Shutdown_File			; 8File
...
gcWinScreenToUse	SET	_Wk_Screen
...
;			DC.B	"SPREAD",0			
			DC.B	-1
 EVEN
_Work_Image:		incbin	"mev3_icons5.img"
PCB:
